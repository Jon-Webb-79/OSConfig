...
rem ------------------------------------------------------------
rem Install shared prjct_name library (tests OFF)
rem - Default prefix: C:\Program Files\prjct_name
rem - Default build type: Release
rem - Uses Ninja if available
rem
rem Usage:
rem   install.bat
rem   install.bat --prefix "C:\prjct_name"
rem   install.bat --relwithdebinfo
rem   install.bat --generator "Ninja"
...
if "%GEN%"=="" (
    cmake -S "%SRC_DIR%" -B "%OUT_DIR%" -DCMAKE_BUILD_TYPE=%BUILD_TYPE% -DCMAKE_INSTALL_PREFIX="%PREFIX%" -DBUILD_SHARED_LIBS=ON -DPRJCT_NAME_BUILD_TESTS=OFF -DPRJCT_NAME_BUILD_STATIC=OFF
) else (
    cmake -S "%SRC_DIR%" -B "%OUT_DIR%" -G "%GEN%" -DCMAKE_BUILD_TYPE=%BUILD_TYPE% -DCMAKE_INSTALL_PREFIX="%PREFIX%" -DBUILD_SHARED_LIBS=ON -DPRJCT_NAME_BUILD_TESTS=OFF -DPRJCT_NAME_BUILD_STATIC=OFF
)
